...
public class FilmDbStorageImpl implements FilmStorage {
    ...
    public List<Film> searchFilms(String query, List<String> searchBy) {
        ...
        if (searchBy.contains("director")) {
            ...
            // Если мы уже добавили параметр для поиска по названию,
            // то добавляем второй параметр в список.
            if (!params.isEmpty()) {
                params.add("%" + query + "%");
                params.add("%" + query + "%"); // Дублируем для второго условия
                // Чтобы избежать дублирования параметра при UNION ALL,
                // лучше использовать другой подход или корректировать SQL-запрос.
                // Например, можно объединить условия в одном запросе:
